...
router.get('/:groupId/history', async (req, res) => {
  try {
    ...
    // Snapshot-driven charting:
    // - Use stored hourly snapshots as source of truth.
    // - If no snapshots exist yet, return a single bootstrap point.
    if (history.length === 0) {
      const initialValue =
        Number(portfolio.initialValue) > 0
          ? Number(portfolio.initialValue)
          : refreshedTotalValue;
      history.push({
        date: new Date(),
        totalValue: refreshedTotalValue,
        cashBalance: Number(portfolio.cashBalance),
        stockValue: Math.max(0, refreshedTotalValue - Number(portfolio.cashBalance)),
        dayChange: 0,
        dayChangePercent: 0,
